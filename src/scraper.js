...
const cls = String(
  typeof n.className === "string"
    ...
    : n.className?.baseVal || ""
      ...
      const missingAltImages = allImages.filter(i => !i.alt && i.src).length;
      const noLazyImages     = allImages.filter(i => i.loading !== "lazy").length;
      ...
      // Touch target checker — buttons/links smaller than 44x44px
      const smallTouchTargets = Array.from(document.querySelectorAll("a, button"))
        .filter(el => {
          const r = el.getBoundingClientRect();
          return r.width > 0 && r.height > 0 && (r.width < 44 || r.height < 44);
        }).length;

      // Duplicate text detector — phrases repeated 3+ times
      const allText = Array.from(document.querySelectorAll("p, h1, h2, h3, li"))
        .map(el => (el.innerText || "").trim().toLowerCase())
        .filter(t => t.length > 20 && t.length < 200);
      const textFreq = {};
      allText.forEach(t => { textFreq[t] = (textFreq[t] || 0) + 1; });
      const duplicateText = Object.entries(textFreq)
        .filter(([, c]) => c >= 3)
        .map(([t]) => t.slice(0, 80))
        .slice(0, 5);

      // Check for social proof keywords
      const pageText = document.body.innerText.toLowerCase();
      const hasSocialProof   = /review|testimonial|rating|stars|clients|customers|trusted by/i.test(pageText);
      const hasPricing       = /price|pricing|per month|\/mo|plan|package|\$|£|€/i.test(pageText);
      const hasPhoneOrEmail  = /\+?\d[\d\s\-()]{7,}|\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i.test(pageText);
      const hasFAQ           = /faq|frequently asked|question/i.test(pageText);
      const hasPrivacyPolicy = /privacy policy|terms of service|terms & conditions/i.test(pageText);
      const hasCookieBanner  = /cookie|gdpr|consent/i.test(pageText);
      ...
      return {
        ...
        buttons:    getText("button,[role='button'],a.btn,a[class*='btn'],a[class*='button'],a[class*='cta']")
                      .filter(t => t.length < 80).slice(0, 8),
        navLinks:   getText("nav a, header a").slice(0, 8),
        images:     allImages.map(i => i.alt).filter(Boolean).slice(0, 10),

        // ── NEW: Technical audit fields ──────────────────────────────
        technical: {
          hasViewportMeta,
          canonicalUrl,
          ogTitle,
          ...
          metaLength:   (document.querySelector('meta[name="description"]')?.content || "").length,
    ...
    // ── Link validator: check internal links for 404s ──────────────────────
    if (!issues || issues.length === 0) {
      try {
        const internalLinks = (content.technical?.allLinks || [])
          .filter(l => {
            try {
        ...
        content.technical.brokenLinks = brokenLinks;
        if (brokenLinks.length) {
          console.log(`🔗 Found ${brokenLinks.length} broken links`);
        }
      } catch (e) {
        content.technical.brokenLinks = [];
        console.log("⚠️ Link check failed:", e.message);
      }
    }
